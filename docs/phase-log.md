...
### Addendum 2026-07-06 (later) — Board rework proposals + scene-composite contract + INCIDENT
- **INCIDENT (honest log):** Ryan's uploaded art had been placed inside assets\_placeholders\ (the generated, gitignored tree). Aletheia ran gen:placeholders without checking for hand-placed files there and OVERWROTE all 388 files - the uploaded copies were destroyed (originals exist with Ryan; re-copy required to assets\<path>). Fixes so it cannot recur: gen-placeholders now NEVER overwrites an existing file without --force ("preserved" counter proves it), and assets\WHERE-ART-GOES.md documents the drop location.
- **DQ-8 (OPEN):** two complete board/clue rework designs delivered for discussion - docs/proposals/board-rework-2026-07-06.md ("The Front Page" vs "The Evidence Wall", shared recommendation: ship discovery feedback first, hide desk/rail until usable).
- **DQ-9 (RESOLVED, canon):** scene-composite model - painted base views + conditional transparent overlay paintings per day/timeframe, paintedCharacters opt-out of sprite actors. Schema (overlays + exclusiveGroup), PixiStage compositing, placeholder generation, founders_square exemplar. Art bible bumped v1.1 -> v1.2 (Ryan-directed canon edit) with the full overlay contract + interview-portrait spec (transparency, facing, NO baked frames/captions - the uploaded Gary portrait carried a baked caption card, which the placeholder layout inadvertently taught; the addendum forbids copying placeholder self-labels).
- Gates: validator PASS, 70/70, autoplay 595 x2, build clean, e2e 5/5.

### Addendum 2026-07-07 — Name pass (DQ-10 RESOLVED, Ryan-directed canon change)
The grandfather IS the first GARY GIBBONS; "Arthur Archie Gibbons" is struck from canon. The protagonist is Gary Gibbons Jr. and is NEVER called "Jr." - the full name "Gary Gibbons," spoken by the town, always means the grandfather; the letter and margin notes are signed "- G.G.", deliberately sharing initials with Gary's own "confirmed - g.g." card stamp (it's hereditary). 82 occurrences swept across content, app UI, docs; design doc bumped v1.3 -> v1.4, art bible v1.2 -> v1.3, CLAUDE.md/ALETHEIA.md canon refs updated. Internal ids (archie_notes pool, archies_desk hotspot, an_* barks) stay frozen per ID FREEZE - invisible to players, save-safe. NG+ feature renamed "Grandpa's Notes" in UI/docs. Gates: validator PASS, 70/70, autoplay 595 x2, build, e2e 5/5.

### Addendum 2026-07-07 (later) — Name pass review round closed
gg-content-reviewer: **LGTM** after 1 round (5 findings, all fixed): doc letter signature "- A.G." straggler -> "- G.G."; credits n6 restored to strict verbatim ("labeled in Gary Gibbons's hand"); design doc now self-announces v1.4 in-file; badge examine single-named; Dot's line gains its head noun ("grandson"). Zero Archie/Arthur/A.G. remain project-wide. FOR RYAN'S EAR: Warren D3 verbatim "...You're Gary Gibbons's, all right. Sit down, Mr. Gibbons." is doc-locked and reads sibilant aloud - your call whether doc+game get a smoother phrasing.
